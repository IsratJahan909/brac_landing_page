# BRAC-Inspired Landing Page

A responsive, modern landing page inspired by [BRAC](https://www.brac.net/) official website.
Built with **HTML5**, **CSS3**, and **Bootstrap 5** to showcase clean design, responsive layout, and user-friendly navigation. This is a personal frontend project for my portfolio.

##  Features
- Fully responsive design (mobile, tablet, desktop)
- Bootstrap 5 grid system, components (navbar, carousel, cards, buttons, forms)
- Hero section with call-to-action (CTA) buttons
- About BRAC section (mission, impact stats)
- Programs/Services sections with cards/grid
- Footer with links, social icons, contact info
- Smooth scrolling and hover effects
- Clean, professional color scheme (inspired by BRAC's green & white)

##  Technologies Used
- HTML5
- CSS3 (Custom styles + Flexbox/Grid)
- Bootstrap 5 (CDN or local)
- Font Awesome / Bootstrap Icons for icons

## Demo / Live Preview
